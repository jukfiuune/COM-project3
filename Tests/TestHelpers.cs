...
{
    private readonly List<Core.Entities.RefreshToken> _tokens = new();

    public Task<Core.Entities.RefreshToken?> GetByTokenAsync(string tokenHash)
    {
        var token = _tokens.FirstOrDefault(t => t.TokenHash == tokenHash);
        return Task.FromResult(token);
    }

    public Task CreateAsync(Core.Entities.RefreshToken token)
    {
    ...
    public Task UpdateAsync(Core.Entities.RefreshToken token)
    {
