...
public sealed class CleanMapReportService(ICleanMapReportRepository repository, IAiDetectionService aiDetectionService) : ICleanMapReportService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var photoBeforeToSave = request.PhotoBefore;
        ...
        if (!string.IsNullOrWhiteSpace(request.PhotoBefore))
        {
            try
            {
                ...
                {
                    var detectedLabels = detections.Select(d => d.label).Distinct();
                    tagsToSave.AddRange(detectedLabels);
                    tagsToSave = tagsToSave.Distinct().ToList();

                    var first = detections.FirstOrDefault();
                    if (first != null && !string.IsNullOrWhiteSpace(first.annotatedImage))
                    {
                        photoBeforeToSave = $"data:image/png;base64,{first.annotatedImage}";
                    }
                }
            }
            catch (Exception)
